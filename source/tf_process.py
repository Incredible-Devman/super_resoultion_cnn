...
def test(frame, sess, neuralnet, saver, dataset):
    # frame = cv2.imread(path)
    # cv2.imwrite("%s/result/bicubic.png" %(PACK_PATH), frame)
    # print(np.shape(frame))
    w, h, c = np.shape(frame)
    # frame = cv2.resize(frame, (DE_HEIGHT, (int)(w * DE_HEIGHT / h)))
    # w, h, c = np.shape(frame)
    # print(np.shape(frame))
    frame = np.reshape(frame, (1, w, h, c))
    frame = frame / 255

    # frame, y = dataset.next_test()

    # print(np.shape(frame))
    if(os.path.exists(PACK_PATH+"/Checkpoint/model_checker.index")):
        saver.restore(sess, PACK_PATH+"/Checkpoint/model_checker")

    makedir(PACK_PATH+"/result")
    makedir(PACK_PATH+"/result/reconstruction")

    start_time = time.time()

    img_recon, tmp_psnr = sess.run([neuralnet.recon, neuralnet.psnr], feed_dict={neuralnet.inputs:frame, neuralnet.outputs:np.zeros(np.shape(frame))})
    img_recon = np.squeeze(img_recon, axis=0)
    # cv2.imshow("original", frame[0])
    cv2.imshow("result", img_recon)
    cv2.waitKey(0)

    plt.imsave("%s/result/reconstruction/psnr_%d.png" %(PACK_PATH, int(tmp_psnr)), img_recon)

    # img_input = np.squeeze(frame, axis=0)
    # img_ground = np.squeeze(Y_te, axis=0)
    # plt.imsave("%s/result/bicubic.png" %(PACK_PATH), img_input)
    # plt.imsave("%s/result/high-resolution.png" %(PACK_PATH), img_ground)
